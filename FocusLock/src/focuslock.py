...
class AppBlocker:
    ...
    def _kill_blocked(self):
        # CREATE_NO_WINDOW stops the CMD flash every 2 seconds
...
class FocusLock(tk.Tk):
    ...
    def _on_close(self):
        ...
        if self._tray_var.get():
            # try pystray first, fall back to pure tkinter mini-window
            if HAS_TRAY:
                self._minimize_to_tray_pystray()
            else:
                self._minimize_to_tray_tkinter()
            return
    ...
    # --- pystray tray (if installed) ---
    def _minimize_to_tray_pystray(self):
        self.withdraw()
        if self._tray is None:
            img = make_tray_icon()
            menu = pystray.Menu(
                pystray.MenuItem("Open FocusLock", self._restore_from_pystray),
                pystray.MenuItem("Quit", self._quit_from_pystray)
            )
            self._tray = pystray.Icon("FocusLock", img, "FocusLock", menu)
            threading.Thread(target=self._tray.run, daemon=True).start()

    def _restore_from_pystray(self, icon=None, item=None):
        self.after(0, self.deiconify)

    def _quit_from_pystray(self, icon=None, item=None):
        if self._locked:
            return
        self.after(0, self._really_quit)

    # --- pure tkinter tray fallback (always works, no extra install) ---
    def _minimize_to_tray_tkinter(self):
        self.withdraw()  # hide main window

        # small floating restore button in bottom-right corner
        restore_win = tk.Toplevel(self)
        restore_win.overrideredirect(True)   # no title bar
        restore_win.attributes("-topmost", True)
        restore_win.attributes("-alpha", 0.92)
        restore_win.configure(bg="#6c63ff")

        # position bottom-right of screen
        sw = self.winfo_screenwidth()
        sh = self.winfo_screenheight()
        restore_win.geometry(f"160x36+{sw - 174}+{sh - 72}")
        ...
        tk.Button(restore_win, text="🔒 FocusLock", command=restore,
                  bg="#6c63ff", fg="white", relief="flat",
                  font=("Segoe UI", 9, "bold"), cursor="hand2", bd=0
                  ).pack(side="left", padx=8, pady=6)

        tk.Button(restore_win, text="✕", command=quit_app,
                  bg="#6c63ff", fg="white", relief="flat",
                  font=("Segoe UI", 9), cursor="hand2", bd=0
                  ).pack(side="right", padx=6)

        self._restore_win = restore_win  # keep reference
