...
from PySide6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel, 
                               QLineEdit, QListWidget, QPushButton, QFileDialog, QWidget, QFrame, QListWidgetItem)
...
class AppPickerDialog(CustomDialog):
    def __init__(self, parent, theme, callback):
        ...
        for cat, items in SUGGESTED_APPS.items():
            ...
            btn.clicked.connect(lambda checked=False, items=items: self._add_multiple(items))
            cats_layout.addWidget(btn)
        cats_layout.addStretch()
        self.layout.addWidget(cats_frame)
        
        # Custom EXE / Bottom Buttons
        ...
        for p in psutil.process_iter(['name', 'exe']):
            try:
                ...
                if name and name.endswith('.exe'):
                    if name.lower() not in ["svchost.exe", "explorer.exe", "taskmgr.exe"]:
...
class SitePickerDialog(CustomDialog):
    def __init__(self, parent, theme, callback):
        ...
        self.layout.addWidget(label("Or choose from common distractions:", "subtext", theme))
        ...
        for cat, domains in SUGGESTED_SITES.items():
            ...
            # Simple flow simulation (just taking first 3-4 for simplicity or putting in horizontal list)
            # A full flow layout is complex in Qt, so we just use HBox for up to 4, then wrap.
            row = QHBoxLayout()
            count = 0
            for domain in domains:
                btn = button(domain, lambda checked=False, d=domain: self._add_domain(d), variant="secondary", theme=theme)
                row.addWidget(btn)
                count += 1
                if count >= 3:
                    row.addStretch()
    ...
    def _add_domain(self, domain):
        ...
        # Optionally show feedback or keep open to add more
        self.site_in.clear()
